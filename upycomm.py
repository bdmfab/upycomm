...
class SLCComm:
    ...
    def __init__(self, plc_ip, plc_port=44818, timeout=5, eth_inf=1):
        print(f"SLCComm {self.VERSION}")
        self.plc_ip = plc_ip
        self.plc_port = plc_port
        self.timeout = timeout
        self.eth_inf = eth_inf 
        self.socket = None
        ...
        # Generate a random serial number for routing
    ...
    def connect_ethernet(self):
        ...
        '''
        if self.eth_inf == 1:
            ''' Configure and activate Ethernet interface on WT32-ETH01 v1.4'''        
            lan = network.LAN(
                mdc=Pin(23),
                mdio=Pin(18),
                power=Pin(16),
            ...
            ''' Initialize SPI bus and configure for EdgeBox-ESP-100 pins using W5500 '''
            spi = SPI(1, 
                baudrate=2000000,
                sck=Pin(13),   # FSPI_SCLK
                mosi=Pin(12),  # FSPI_DO
                miso=Pin(11))  # FSPI_DI
